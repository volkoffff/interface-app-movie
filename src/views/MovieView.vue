...
async function updateMovieTitle() {
  if (ModifyState) {
    if (informationsDuEnfant.value) {
      formattedArray.value = informationsDuEnfant.value.map(
        (id) => `/api/actors/${id}`
      );
    }
    ...
    try {
      ...
      const updatedMovie = reactive({
        ...
        releaseDate: editedMovieDate.value,
        category: `/api/categories/${selectedValue.value}`,
        actors: formattedArray,
  ...
  <div v-if="data">
    <div class="banner-movie">
      <div class="banner-movie-left">
        <div class="baner-movie-image-container">
          <img
            src="https://fr.web.img5.acsta.net/pictures/23/04/03/15/05/1583867.jpg/r_5000_x"
          />
        </div>
      ...
      <div class="banner-movie-right"></div>
    </div>
      ...
      <div>
        <div v-if="allMoviesData" class="container-list">
          <moviesCard
            ...
            :id="movie.id"
            :movie="movie"
            v-show="data.category.name == movie.category.name"
          />
        </div>
...
.top-movie {
  ...
  background: linear-gradient(rgba(0, 0, 0, 0), var(--black-background) 70%),
    url(https://fr.web.img5.acsta.net/pictures/23/04/03/15/05/1583867.jpg/r_5000_x);
  background-size: 120%;
